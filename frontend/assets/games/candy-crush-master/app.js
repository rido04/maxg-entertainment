document.addEventListener("DOMContentLoaded", () => {
    ...
    function handleTouchMove(e) {
        e.preventDefault();
        // Opsional: Tambah visual drag effect
    }
    ...
    function handleTouchEnd(e) {
        ...
        const threshold = 30; // swipe minimal jarak
        let targetId = null;

        if (Math.abs(deltaX) > Math.abs(deltaY)) {
            if (deltaX > threshold) targetId = squareIdBeingDragged + 1;
            else if (deltaX < -threshold) targetId = squareIdBeingDragged - 1;
        } else {
            if (deltaY > threshold) targetId = squareIdBeingDragged + width;
            else if (deltaY < -threshold)
                targetId = squareIdBeingDragged - width;
        }

        if (targetId !== null && squares[targetId]) {
            colorBeingReplaced = squares[targetId].style.backgroundImage;
