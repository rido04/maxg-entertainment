...
<div class="min-h-screen text-gray-700 bg-gray-200">
    ...
    <div class="px-6 pb-8">
        <div class="mb-6">
            <h2 class="text-2xl font-bold text-gray-700 mb-4">Popular</h2>
                <div class="flex items-center space-x-3">
                    <button id="play-all-btn" onclick="playAllSongs()" class="flex items-center space-x-2 bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-full transition-all duration-200 font-medium">
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z" clip-rule="evenodd" />
                        </svg>
                        <span>Play All</span>
                    </button>
                    <button id="shuffle-play-btn" onclick="shufflePlayAll()" class="flex items-center space-x-2 bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-full transition-all duration-200 font-medium">
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
...
function playAllSongs() {
    if (window.artistPlayer && window.artistPlayer.playlist.length > 0) {
        // Mulai dari lagu pertama
        window.artistPlayer.playSong(0);
        
        // Update UI untuk menunjukkan bahwa sedang play all
        ...
        if (playAllBtn) {
            ...
            // Kembalikan tampilan tombol setelah beberapa detik
            setTimeout(() => {
                playAllBtn.innerHTML = `
                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z" clip-rule="evenodd" />
...
// Tambahkan juga function untuk shuffle play all (optional)
function shufflePlayAll() {
    if (window.artistPlayer && window.artistPlayer.playlist.length > 0) {
        // Buat array index lagu yang diacak
        const shuffledIndexes = [...Array(window.artistPlayer.playlist.length).keys()]
            .sort(() => Math.random() - 0.5);
        
        // Mulai dari lagu pertama yang sudah diacak
        window.artistPlayer.playSong(shuffledIndexes[0]);
        
        // Set playlist order untuk shuffle
        window.artistPlayer.shuffleMode = true;
        window.artistPlayer.shuffledIndexes = shuffledIndexes;
    }
}
</script>
